...
public class windowCloser {
    ...
    public static void display(Stage a1) {
        ...
        window.setTitle("Close Window?");//Giving the window a title
        window.setMinWidth(250);//setting the size
        window.setMinHeight(125);

        Label label1 = new Label();//creating labels
        label1.setText("Are you sure you want to exit?");//Giving a text to the labels

        Button b1 = new Button("Yes");
        Button b2 = new Button("No");

        b1.setOnAction(e->{window.close();a1.close();});
